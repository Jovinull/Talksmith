...
class ChatUI:
    def __init__(self, corpus_path: str, k: int = 3, threshold: float = 0.1):
        ...
        self.chat_area = scrolledtext.ScrolledText(
            self.window, wrap=tk.WORD, font=self.font,
            bg=self.chat_bg, fg=self.bot_text_color,
        ...
        self.entry = tk.Entry(
            self.entry_frame, font=self.font,
            bg=self.input_bg, fg="white", insertbackground="white", relief="flat"
        )
        self.entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,10))
        self.entry.bind("<Return>", self.send_message)

        self.send_button = tk.Button(
            self.entry_frame, text="Enviar", font=self.font,
    ...
    def display_message(self, sender: str, message: str):
        ...
        color = self.user_text_color if prefix=="Você" else self.bot_text_color
